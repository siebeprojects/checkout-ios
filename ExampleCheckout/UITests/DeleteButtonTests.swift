...
final class DeleteButtonTests: NetworksTests {
    ...
    func testDeleteButton_whenUpdateFlow_whenAllowDeleteIsTrue_shouldShow() throws {
        try XCTContext.runActivity(named: "Delete the payment method") { _ in
            let transaction = try Transaction(operationType: .update, allowDelete: true, customerId: Self.customerId)
    ...
    func testDeleteButton_whenUpdateFlow_whenAllowDeleteIsFalse_shouldHide() throws {
        try XCTContext.runActivity(named: "Delete the payment method") { _ in
            let transaction = try Transaction(operationType: .update, allowDelete: false, customerId: Self.customerId)
            try setupPaymentSession(transaction: transaction)
    ...
    func testDeleteButton_whenUpdateFlow_whenAllowDeleteIsNil_shouldShow() throws {
        try XCTContext.runActivity(named: "Delete the payment method") { _ in
            let transaction = try Transaction(operationType: .update, allowDelete: nil, customerId: Self.customerId)
            try setupPaymentSession(transaction: transaction)
    ...
    func testDeleteButton_whenChargeFlow_whenAllowDeleteIsTrue_shouldShow() throws {
        try XCTContext.runActivity(named: "Delete the payment method") { _ in
            let transaction = try Transaction(operationType: .charge, allowDelete: true, customerId: Self.customerId)
            try setupPaymentSession(transaction: transaction)
    ...
    func testDeleteButton_whenChargeFlow_whenAllowDeleteIsFalse_shouldHide() throws {
        try XCTContext.runActivity(named: "Delete the payment method") { _ in
            let transaction = try Transaction(operationType: .charge, allowDelete: false, customerId: Self.customerId)
            try setupPaymentSession(transaction: transaction)
    ...
    func testDeleteButton_whenChargeFlow_whenAllowDeleteIsNil_shouldHide() throws {
        try XCTContext.runActivity(named: "Delete the payment method") { _ in
            let transaction = try Transaction(operationType: .charge, allowDelete: nil, customerId: Self.customerId)
            try setupPaymentSession(transaction: transaction)
